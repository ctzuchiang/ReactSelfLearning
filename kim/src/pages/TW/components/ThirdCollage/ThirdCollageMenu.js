...
export default function ThirdCollageMenu({ dalogId, setDalogId }) {
    ...
    const [SymbolSelected, setSymbolSelected] = React.useState(false);
    const [PoliticalFiguresSelected, setPoliticalFiguresSelected] = React.useState(false);

    React.useEffect(() => {
        if (dalogId === "Symbol" || dalogId === "SymbolExample") {
        ...
        if (
            ...
            setPoliticalFiguresSelected(true);
        } else {
            setPoliticalFiguresSelected(false);
        }
    }, [dalogId]);
    return (
        <React.Fragment>
            <ToggleButtonGroup exclusive value={dalogId} onChange={handleToggleChange}>
                <DailogToggleButton selected={SymbolSelected} value="Symbol">
                    <b>Symbol</b>
                </DailogToggleButton>
                <DailogToggleButton selected={PoliticalFiguresSelected} value="PoliticalFigures">
                    <b>Political Figures</b>
                </DailogToggleButton>
                <DailogToggleButton value="EdibleStatement">
