...
export default function FiveCollageMenu({ dalogId, setDalogId }) {
    ...
    const [SpiritualitySelected, setSpiritualitySelected] = React.useState(false);

    React.useEffect(() => {
        if (dalogId === "Spirituality" || dalogId === "FortuneTelling") {
            setSpiritualitySelected(true);
        } else {
            setSpiritualitySelected(false);
        }
    }, [dalogId]);
    ...
    return (
        <React.Fragment>
            <ToggleButtonGroup exclusive value={dalogId} onChange={handleToggleChange}>
                <DailogToggleButton selected={SpiritualitySelected} value="Spirituality">
                    <b>Spirituality</b>
                </DailogToggleButton>
            </ToggleButtonGroup>
        </React.Fragment>
